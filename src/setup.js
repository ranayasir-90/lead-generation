...
class SetupWizard {
    ...
    async configurePreferences() {
        ...
        this.profile.preferences.language = 'english';
        ...
        const suggestion = bizType ? `${bizType} Islamabad` : '';
        this.profile.preferences.defaultSearchQuery = await this.question(
            `Default search query${suggestion ? ` (e.g., "${suggestion}")` : ''}: `
        ) || '';
        
        // Default location
        this.profile.preferences.defaultLocation = await this.question('Default target location (e.g., Islamabad): ') || '';
    }
...
`;
        ...
        await save(this.profile);
        console.log(`✅ Business profile saved to Supabase`);
        
        // Create user preferences file and save to Supabase
        const prefsContent = {
        ...
        await db.saveUserPreferences(prefsContent);
    ...
    async runSampleCampaign() {
        ...
        if (runSample.toLowerCase() === 'y' || runSample.toLowerCase() === 'yes') {
            ...
            const location = this.profile.preferences.defaultLocation || 'Islamabad';
