...
class DataTable {
            ...
            <table class="leads-table">
                ...
                <tbody>
                    ${pageData.length > 0 ? pageData.map((row, index) => `
                        <tr class="clickable-row" data-row-index="${startIndex + index}">
                            ${this.options.columns.map(col => {
    ...
    attachEventListeners() {
        ...
        // Clickable rows
        this.container.querySelectorAll('.clickable-row').forEach(rowEl => {
            rowEl.addEventListener('click', (e) => {
                // Ignore click if it's on interactive elements like links, dropdown selects, and buttons
                if (e.target.closest('a, select, button, input, label')) {
                    return;
                }
                const rowIndex = parseInt(rowEl.dataset.rowIndex);
                if (this.options.onRowClick && !isNaN(rowIndex)) {
                    this.options.onRowClick(this.filteredData[rowIndex], rowIndex);
                }
            });
        });
    }
...
class ProgressManager {
    ...
    complete(results) {
        ...
        const highScoredCount = results.highScoreLeads !== undefined ? results.highScoreLeads : (results.priorityLeads || 0);
        this.addMessage(`Generated ${results.totalLeads} leads with ${highScoredCount} high quality prospects`, 'success');
        this.closeButton.disabled = false;
    }
